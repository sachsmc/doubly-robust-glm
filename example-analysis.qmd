...
infunc_confint <- function(ofit, wfit, exposurename, outcomename) {
  ...
  Kterm1 <- (-1/n) * matrix(((data[[exposurename]] * hdot) / phat^2) * (data[[outcomename]] - est1), nrow = 1, ncol = n) %*%
    model.matrix(wfit)
  Kterm0 <- (1/n) * matrix((((1 - data[[exposurename]]) * hdot) / (1 - phat)^2) * (data[[outcomename]] - est0), nrow = 1, ncol = n) %*%
    model.matrix(wfit)
